...
namespace FeatureTracker.Server.Controllers.Authentication;
...
public class AuthController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(string), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<bool>> InviteUser(string email)
    {
        try
        {
            return Ok(await _authApplication.PreRegisterUserAsync(email.ToLower(), this.GetUserAuthenticatedId()));
        }
        catch (Exception ex)
        {
    ...
    [AllowAnonymous]
    public async Task<ActionResult<UserRegisterViewModel>> ValidateToken(string token)
    {
        try
        {
